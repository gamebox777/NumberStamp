...
### 6.8 開発者モード (Developer Mode)

| ID | 要件 | 優先度 |
| :--- | :--- | :--- |
| DEV-001 | バージョン番号を5回連続クリック(2秒以内)することで開発者モードのON/OFFを切り替えられること | 必須 |
| DEV-002 | 開発者モードON時、画面上にデバッグ情報を表示するオーバーレイが出現すること | 必須 |
| DEV-003 | デバッグ情報として、マウス座標、ズーム倍率、アイテム数、選択ID、Z-Order、タイプ、サイズ、現在のモードを表示すること | 必須 |
...
| 2026-02-19 | 1.1 | 開発者モード（デバッグ表示機能）の追加、拡張（Z-Order等） |
